...
const Keyboard = ({ lower = defaultLower, upper = defaultUpper }: { lower?: Pitch; upper?: Pitch }) => {
  if (comparePitches(lower, upper) > 0) {
    throw new Error('Lower pitch must be lower than upper pitch')
  }
  const pitches = generatePianoPitchesWithSharp().filter((pitch) => {
    return comparePitches(pitch, lower) >= 0 && comparePitches(pitch, upper) <= 0
  })
